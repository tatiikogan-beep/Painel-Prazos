    ...
    if st.button("🚀 Gerar Relatório", type="primary", use_container_width=True):
        ...
        if tem_alerta:
            ...
            for tipo, dados in alertas:
                ...
                if tipo == 'NAO_MAPEADO':
                    with st.expander(
                        f"🔴 {len(dados)} executante(s) NÃO MAPEADO(S) — ver aba INCONSISTENCIAS no relatório",
                        expanded=True
                    ):
                        st.warning(
                            "Os executantes abaixo **não foram reconhecidos** pelo sistema. "
                            "Suas linhas foram incluídas na **aba INCONSISTENCIAS** do relatório, "
                            "com os dados disponíveis e sem preenchimento do responsável. "
                            "Informe a gestão do sistema para incluí-los no cadastro.",
                        )
                        import pandas as pd
                        df_nm = pd.DataFrame(
                            [(row_v.get('Processo',''), exe, row_v.get('Data',''),
                              str(row_v.get('Descricao') or '')[:80])
                             for row_v, exe in dados],
                            columns=["Processo", "Executante", "Data", "Descrição"]
                        )
        ...
        c1.metric("Total base", resumo.get('total_geral', resumo['total']))
        c2.metric("🟠 Laranjas", resumo['laranjas'])
        c3.metric("🔴 Erros de data", resumo['data_erros'])
        c4.metric("❌ Inconsistências", resumo['nao_mapeados'])
