...
def gerar_relatorio(input_source):
    ...
    for row in rows_in:
        ...
        if supervisor:
            ...
            nao_mapeados.append((row_out, str(exec_raw or '')))
    ...
    if nao_mapeados:
        ws_r.cell(L, 1,
            ...
            f'ver aba INCONSISTENCIAS:'
        ).font = Font(bold=True, size=11, color='CC0000'); L += 1

        H_NM = ['Executante (nao reconhecido)', 'Numero do Processo',
                 'Data', 'Descricao', 'Acao necessaria']
        ...
        for row_v, exe in nao_mapeados:
            proc = row_v.get('Processo', '')
            data = row_v.get('Data', '')
            desc = str(row_v.get('Descricao') or '')[:100]
            vals = [exe, proc, data, desc,
    ...
    # ═══════════════════════════════════════════════════════════════════
    # ABA INCONSISTENCIAS — todos os prazos sem executor mapeado
    # ═══════════════════════════════════════════════════════════════════
    if nao_mapeados:
        ...
        # Larguras
        for j, (nome, larg) in enumerate(COLUNAS_SAIDA, 1):
            ws_inc.column_dimensions[get_column_letter(j)].width = larg
        ws_inc.column_dimensions[get_column_letter(len(COLUNAS_SAIDA) + 1)].width = 45

        # Freeze e autofiltro
        ws_inc.freeze_panes = 'A2'

        # Cabeçalho: colunas padrão + coluna de observação
        cabec_inc = NOMES_COLUNAS + ['Observacao']
        for j, h in enumerate(cabec_inc, 1):
            _cel_d(ws_inc, 1, j, h, F_VERMELHO, FONTE_BRANCA, AL_C_WRAP)

        # Dados — fundo rosa claro para sinalizar inconsistência
        F_INC = PatternFill('solid', fgColor='FFE0E0')
        for li, (row_v, exe) in enumerate(nao_mapeados, 2):
            for j, nome_col in enumerate(NOMES_COLUNAS, 1):
                v = row_v.get(nome_col)
                disp = None if str(v or '').strip() in ('nan','None','') else v
                c2 = _cel_d(ws_inc, li, j, disp, F_INC, FONTE_NORMAL, AL_E_WRAP)
            # Coluna Observacao
            obs = (f'Executante "{exe}" nao mapeado — '
                   'Responsavel e supervisor nao identificados. '
                   'Comunicar gestao do sistema.')
            _cel_d(ws_inc, li, len(NOMES_COLUNAS) + 1, obs,
                   F_INC, Font(size=11, color='CC0000'), AL_E_WRAP)

        ultima_col_inc = get_column_letter(len(cabec_inc))
        ws_inc.auto_filter.ref = f'A1:{ultima_col_inc}1'

    # ── Retorno ───────────────────────────────────────────────────────
    alertas = []
    if nao_mapeados:
        alertas.append(('NAO_MAPEADO',
                         [(row_v, exe) for row_v, exe in nao_mapeados]))
    if erros_data:
        alertas.append(('DATA_DIVERGENTE', [
            (r[1], r[2], r[3]) for r in erros_data
        ]))
    if colunas_novas:
        alertas.append(('COLUNA_NOVA', colunas_novas))

    cnt_total = _cnt(processadas)
    total_geral = cnt_total['total'] + len(nao_mapeados)
    resumo = {
        ...
        'alertas':            alertas,
        'nao_mapeados_lista': [(row_v.get('Processo',''), exe)
                               for row_v, exe in nao_mapeados],
